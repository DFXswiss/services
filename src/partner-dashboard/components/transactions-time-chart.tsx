...
export function TransactionsTimeChart({ timeline, theme }: TransactionsTimeChartProps): JSX.Element {
  ...
  const series = useMemo(
    () => [
      {
        name: seriesLabels.buy,
        data: timelineSeriesValues(timelineSeries(buckets, 'transactions', 'buy')),
      },
      {
        name: seriesLabels.sell,
        data: timelineSeriesValues(timelineSeries(buckets, 'transactions', 'sell')),
      },
      {
        name: seriesLabels.swap,
        data: timelineSeriesValues(timelineSeries(buckets, 'transactions', 'swap')),
      },
    ],
  ...
  return (
    <section className="partner-card partner-chart-root" data-testid="transactions-time-chart">
      <h2 className="text-sm font-semibold mb-1" style={{ color: 'var(--text)' }}>
        {title}
      </h2>
      <p className="text-2xs partner-text-secondary mb-2" data-testid="transactions-chart-description">
        {description}
      </p>
      {!hasData ? (
