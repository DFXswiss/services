...
class Eth {
  ...
  async signTransaction(path, rawTxHex) {
    ...
    let rlpTx = _ethers.ethers.utils.RLP.decode("0x" + rawTxHex).map(hex => Buffer.from(hex.slice(2), "hex"));

    let rlpOffset = 0;
    let chainIdPrefix = "";

    if (rlpTx.length > 6) {
      let rlpVrs = Buffer.from(_ethers.ethers.utils.RLP.encode(rlpTx.slice(-3)).slice(2), "hex");
    ...
    rlpTx = _ethers.ethers.utils.RLP.decode("0x" + rawTxHex);
    const decodedTx = {
      data: rlpTx[5],
      to: rlpTx[3]
    };
    ...
    const provideForContract = async address => {
      const erc20Info = (0, _erc.byContractAddress)(address);

      if (erc20Info) {
        (0, _logs.log)("ethereum", "loading erc20token info for " + erc20Info.contractAddress + " (" + erc20Info.ticker + ")");
    ...
    if (decodedTx.data.length >= 10) {
      ...
      const infos = (0, _contracts.getInfosForContractMethod)(decodedTx.to, selector);
      ...
      if (infos) {
        ...
        if (plugin) {
          (0, _logs.log)("ethereum", "loading plugin for " + selector);
          await setExternalPlugin(this.transport, payload, signature);
        }

        if (erc20OfInterest && erc20OfInterest.length && abi) {
          const contract = new _ethers.ethers.utils.Interface(abi);
        ...
        (0, _logs.log)("ethereum", "no infos for selector " + selector);
      }
