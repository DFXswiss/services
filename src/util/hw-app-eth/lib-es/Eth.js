...
export default class Eth {
  ...
  async signTransaction(path, rawTxHex) {
    ...
    let rlpTx = ethers.utils.RLP.decode("0x" + rawTxHex).map(hex => Buffer.from(hex.slice(2), "hex"));
    let rlpOffset = 0;
    let chainIdPrefix = "";

    if (rlpTx.length > 6) {
      let rlpVrs = Buffer.from(ethers.utils.RLP.encode(rlpTx.slice(-3)).slice(2), "hex");
