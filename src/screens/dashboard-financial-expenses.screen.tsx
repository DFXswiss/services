...
function RefDetailChart({ entries }: { entries: FinancialChangesEntry[] }) {
  ...
  const series = useMemo(() => [
    { name: 'Ref Amount', data: entries.map((e) => [new Date(e.timestamp).getTime(), Math.round(e.minus.ref.amount)]) },
...
function BinanceDetailChart({ entries }: { entries: FinancialChangesEntry[] }) {
  ...
  const series = useMemo(() => [
    { name: 'Trading', data: entries.map((e) => [new Date(e.timestamp).getTime(), Math.round(e.minus.binance.trading)]) },
    { name: 'Withdraw', data: entries.map((e) => [new Date(e.timestamp).getTime(), Math.round(e.minus.binance.withdraw)]) },
  ], [entries]);
...
function BlockchainDetailChart({ entries }: { entries: FinancialChangesEntry[] }) {
  ...
  const series = useMemo(() => [
    { name: 'TX Out', data: entries.map((e) => [new Date(e.timestamp).getTime(), Math.round(e.minus.blockchain.txOut)]) },
    { name: 'TX In', data: entries.map((e) => [new Date(e.timestamp).getTime(), Math.round(e.minus.blockchain.txIn)]) },
    { name: 'Trading', data: entries.map((e) => [new Date(e.timestamp).getTime(), Math.round(e.minus.blockchain.trading)]) },
  ], [entries]);
...
export default function DashboardFinancialExpensesScreen(): JSX.Element {
  ...
  const { getFinancialChanges } = useDashboard();
