...
- Transaction information
  ...
  - Personal IBAN provider (`personal-iban`): explicit personal IBAN bank selection for buy (e.g. `frick` for Bank Frick). Example: `/buy?asset-in=EUR&personal-iban=frick`. Requires EUR, bank payment, and KYC50. Selection is fail-closed: unknown values are rejected by the API and do not fall back to the default bank.
